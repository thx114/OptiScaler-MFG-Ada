...
std::atomic<UINT64> bridgeAppliedEpoch { 0 };
...
void DlssNr_Dx12::ApplyFinishedBridge(ID3D12Resource* picture, ID3D12CommandQueue* queue, DXGI_COLOR_SPACE_TYPE space)
{
    std::lock_guard lock(_state->mutex);
    // Called from Dx11wDx12SC::Present before the FG present: the picture is a real game frame.
...
// Set bridgeAppliedEpoch so the FG present hook (which fires synchronously inside the
// following Present) skips its own NR call — one frame must not be processed twice.
